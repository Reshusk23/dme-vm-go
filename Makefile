...
build-c-contracts:
	...
	moapy contract build ./test/contracts/reshuskei
...
ifndef SANDBOX
	...
	rm -rf ${SANDBOX}/dme-delegation-rs
	git clone --depth=1 --branch=master https://github.com/Reshusk23/dme-delegation-rs.git ${SANDBOX}/dme-delegation-rs
	rm -rf ${SANDBOX}/dme-delegation-rs/.git
	moapy contract build ${SANDBOX}/dme-delegation-rs
	moapy contract test --directory="tests" ${SANDBOX}/dme-delegation-rs
	cp ${SANDBOX}/dme-delegation-rs/output/delegation.wasm ./test/delegation/delegation.wasm
...
ifndef SANDBOX
	...
	git clone --depth=1 --branch=master https://github.com/Reshusk23/dme-dns-rs.git ${SANDBOX}/dme-dns-rs
...
build-dme-examples-rs:
ifndef SANDBOX
	$(error SANDBOX variable is undefined)
endif
	rm -rf ${SANDBOX}/dme-examples-rs
	
	moapy contract new --template=simple-coin --directory ${SANDBOX}/dme-examples-rs simple-coin
	moapy contract new --template=adder --directory ${SANDBOX}/dme-examples-rs adder
	moapy contract build ${SANDBOX}/dme-examples-rs/adder
	moapy contract build ${SANDBOX}/dme-examples-rs/simple-coin
	moapy contract test ${SANDBOX}/dme-examples-rs/adder
	moapy contract test ${SANDBOX}/dme-examples-rs/simple-coin
	cp ${SANDBOX}/dme-examples-rs/adder/output/adder.wasm ./test/adder/adder.wasm
	cp ${SANDBOX}/dme-examples-rs/simple-coin/output/simple-coin.wasm ./test/erc20/contracts/simple-coin.wasm
