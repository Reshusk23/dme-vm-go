...
import (
	...
	mj "github.com/Reshusk23/dme-util/test-util/mandos/json/model"
...
func (ae *ArwenTestExecutor) validatorRewardOutput(tx *mj.Transaction) (*vmi.VMOutput, error) {
	...
	storageReshuskReward := recipient.StorageValue(ReshuskRewardKey)
	storageReshuskReward = big.NewInt(0).Add(
		big.NewInt(0).SetBytes(storageReshuskReward),
		reward).Bytes()
	...
	outputAccounts[string(tx.To.Value)] = &vmcommon.OutputAccount{
		...
		StorageUpdates: map[string]*vmi.StorageUpdate{
			ReshuskRewardKey: &vmi.StorageUpdate{
				Offset: []byte(ReshuskRewardKey),
				Data:   storageReshuskReward,
			},
		},
	}
