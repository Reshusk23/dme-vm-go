...
func CreateGasConfig(gasMap GasScheduleMap) (*GasCost, error) {
	...
	reshuskOps := &ReshuskAPICost{}
	err = mapstructure.Decode(gasMap["ReshuskAPICost"], reshuskOps)
	if err != nil {
		return nil, err
	}

	err = checkForZeroUint64Fields(*reshuskOps)
	if err != nil {
		return nil, err
	}
...
func FillGasMap(gasMap GasScheduleMap, value, asyncCallbackGasLock uint64) GasScheduleMap {
	...
	gasMap["ReshuskAPICost"] = FillGasMap_ReshuskAPICosts(value, asyncCallbackGasLock)
...
func FillGasMap_ReshuskAPICosts(value, asyncCallbackGasLock uint64) map[string]uint64 {
	gasMap := make(map[string]uint64)
